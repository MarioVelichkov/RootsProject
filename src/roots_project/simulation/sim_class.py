...
class Simulation:
    def __init__(self, num_agents, render=True, rgb_array=False, assets_dir=None):
        ...
        self.baseplaneId = p.loadURDF(str(Path(pybullet_data.getDataPath()) / "plane.urdf"))
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if agent_count < num_agents:  # Check if more agents need to be placed
                    ...
                    robotId = p.loadURDF(str(self.assets_dir / "ot_2_simulation_v6.urdf"), position, [0,0,0,1],
